...
<div class="page-container">
    ...
    <div class="main-content">
        ...
        <div style="display: flex;align-items: center;justify-content: space-between">
            <h2>لیست نمونه کار ها</h2>

            <a href="<?= admin_uri('/portfolios/create') ?>" class="btn btn-success btn-sm btn-icon icon-left">
        ...
        <table class="table table-bordered table-striped datatable" id="table-2">
            ...
            <?php foreach ($data['portfolios'] as $portfolio) {
                ...
                <tr>
                    ...
                    <td><?= substr($portfolio->description, 0, 30) . '...' ?></td>
                    <td><?= $portfolio->slug ?></td>
                    <td><a href="<?= asset('/images/portfolio/' . $portfolio->cover) ?>" target="_blank">مشاهده</a>
                    </td>
                    <td><?= date('Y-m-d', strtotime($portfolio->date)) ?></td>
                    <td><?= $portfolio->client_name ?></td>
                    <td><?= $portfolio->location ?></td>
                    <td><?= $portfolio->website ?></td>
                    <td><?= date('Y-m-d', strtotime($portfolio->created_at)) ?></td>
                    <td>
                        <a href="<?= admin_uri('/portfolios/edit/' . $portfolio->id) ?>"
                           class="btn btn-default btn-sm btn-icon icon-left">
                            <i class="entypo-cancel"></i>
                            ویرایش
                        </a>

                        <a href="<?= admin_uri('/portfolios/delete/' . $portfolio->id) ?>"
