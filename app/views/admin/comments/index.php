    ...
    <div class="main-content">
        ...
        <ol class="breadcrumb bc-3">
            ...
            <li>
                ...
                <a href="<?= admin_uri('/comments') ?>">کامنت ها</a>
            </li>
            <li class="active">
        ...
        <table class="table table-bordered table-striped datatable" id="table-2">
            ...
            <?php foreach ($data['comments'] as $comment) {
                ...
                <tr>
                    ...
                    <td class="<?= $comment->is_approved ? 'text-success' : 'text-danger' ?>"><?= $comment->is_approved ? 'تایید شده' : 'تایید نشده' ?></td>
                    <td><?= date('Y-m-d', strtotime($comment->created_at)) ?></td>
                    <td>
                        <div style="display: flex;align-items: center;gap: 5px;">
                            <form action="<?= admin_uri('/comments/status/' . $comment->id) ?>" method="post"
                                  class="d-inline-block">
                                <button type="submit" class="btn btn-default btn-sm btn-icon icon-left d-inline-block">
                                    <i class="entypo-pencil"></i>
                                    تغییر وضعیت
                                </button>
                            </form>

                            <a href="<?= admin_uri('/comments/delete/' . $comment->id) ?>"
