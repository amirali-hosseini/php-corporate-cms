...
<div class="page-container">
    ...
    <div class="main-content">
        ...
        <div style="display: flex;align-items: center;justify-content: space-between">
            <h2>لیست نظرات مشتریان</h2>

            <a href="<?= admin_uri('/testimonials/create') ?>" class="btn btn-success btn-sm btn-icon icon-left">
        ...
        <table class="table table-bordered table-striped datatable" id="table-2">
            ...
            <?php foreach ($data['testimonials'] as $testimonial) {
                ...
                <tr>
                    <td>
                        <div class="checkbox checkbox-replace">
                            <input type="checkbox" id="<?= $testimonial->id ?>">
                        </div>
                    </td>
                    <td><?= $testimonial->title ?></td>
                    <td><?= substr($testimonial->description, 0, 60) . '...' ?></td>
                    <td><?= $testimonial->client_name ?></td>
                    <td><?= $testimonial->client_job_title ?></td>
                    <td><a href="<?= uri('/public/assets/images/clients/' . $testimonial->client_photo) ?>"
                           target="_blank">مشاهده</a>
                    </td>
                    <td><?= date('Y-m-d', strtotime($testimonial->created_at)) ?></td>
                    <td>
                        <a href="<?= admin_uri('/testimonials/edit/' . $testimonial->id) ?>"
                           class="btn btn-default btn-sm btn-icon icon-left">
                            <i class="entypo-cancel"></i>
                            ویرایش
                        </a>

                        <a href="<?= admin_uri('/testimonials/delete/' . $testimonial->id) ?>"
